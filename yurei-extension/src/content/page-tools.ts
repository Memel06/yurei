import {
  ...
  type SetValueResult, type TextResult, type TreeOptions, type TreeResult,
} from "../page-api";
...
(() => {
  ...
  function tree(options: TreeOptions): TreeResult {
    ...
    let text = lines.join("\n");
    if (truncated) text += `\n[stopped after ${MAX_NODES} elements; pass ref to focus on a part of the page]`;
    return { ok: true, text: truncateText(text, options.maxChars, "pass a larger max_chars or a ref to focus"), viewport: viewport(), frames };
  }
  ...
  function text(maxChars: number): TextResult {
    const full = (document.body?.innerText ?? "").replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").trim();
    return { ok: true, text: truncateText(full, maxChars, "pass a larger max_chars"), total: full.length };
  }
