...
Rules: refs change when the page changes, so take them from the latest result and never guess. Cookie banners with a reject button are rejected for you; when a result says a banner has no reject button, stop and ask the user to dismiss it themselves or to tell you which button to press, and never accept cookies on your own. If something else covers the page, look for its close button. To work in a tab the user already has open, call tabs_context and pass its tabId. Ask the user before logging in, paying, sending messages or deleting anything. If a result says the user pressed Stop, stop and wait for them.`;
